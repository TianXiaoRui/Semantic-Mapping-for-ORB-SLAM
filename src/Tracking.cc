...
Tracking::Tracking(System *pSys,//指针
        ...
        FrameDrawer *pFrameDrawer,//帧huizi
        MapDrawer *pMapDrawer,//map
        Map *pMap,//map
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, const cv::Mat &imSeg)//imseg传入的是灰度图
{
    mImGray = im;
    imSeg.copyTo(mImSeg);
    //cv::Mat im2;
    ...
    {
        if(mbRGB)
            cvtColor(mImGray,mImGray,CV_RGB2GRAY);//rgb->gray
        else
            cvtColor(mImGray,mImGray,CV_BGR2GRAY);//bgr->gray
    }
    else if(mImGray.channels()==4)
    {
        if(mbRGB)
...
void Tracking::Track()
{
    ...
    if(mState==NOT_INITIALIZED)
    {
        ...
        mpFrameDrawer->Update(this);//这里是进行帧绘制的入口程序

        if(mState!=OK)
            return;//若初始化没有完成，则返回继续执行初始化，现在mstate=ok，初始化已经完毕了,继续else部分
    }
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                {
                    bOK = TrackWithMotionModel();// for cars with uniform motion
                    if(!bOK)
                        bOK = TrackReferenceKeyFrame();
                }
            }
...
void Tracking::CheckReplacedInLastFrame()//检查更新上一帧被替换的地图点
{
